...
# UI Title
st.set_page_config(page_title="GLiNER Named Entity Extractor", layout="wide")
st.title("🔍 GLiNER Entity Extractor for PII")
...
# Label Selector
selected_labels = st.multiselect("🏷️ Select entity labels to detect:", options=ALL_LABELS, default=["person", "organization", "phone number", "email", "social security number","address"])
...
if st.button("🚀 Extract Entities"):
    if text_input.strip() == "":
        ...
        with st.spinner("Detecting entities..."):
            entities = model.predict_entities(text_input, selected_labels)

        if entities:
            st.success(f"✅ Found {len(entities)} entit{'ies' if len(entities) > 1 else ''}.")
            # Display as a table
            st.dataframe([{ "Entity Text": ent["text"], "Label": ent["label"] } for ent in entities])
        else:
            st.info("No entities found with the selected labels.")
